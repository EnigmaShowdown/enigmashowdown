...
object ZigZagMoveAI : StateListener {

    private val moveActions = listOf(
        MoveAction(Math.toRadians(0.0), 5.0),
        MoveAction(Math.toRadians(30.0), 5.0),
        MoveAction(Math.toRadians(180.0), 5.0),
        MoveAction(Math.toRadians(-90.0), 5.0),
        MoveAction(Math.toRadians(90.0 + 45.0), 5.0),
        MoveAction(Math.toRadians(-4.0), 5.0),
        MoveAction(Math.toRadians(70.0), 5.0),
        MoveAction(Math.toRadians(180.0 + 45.0), 8.0),
    )
    ...
    override fun onBroadcast(playerId: UUID, levelStateBroadcast: LevelStateBroadcast, client: RequestClient) {
        ...
        client.send(
            PlayerActionRequest(
                ...
                state.tick,
                ConquestAction(
                    moveAction = moveActions[(state.tick / (3 * EnigmaShowdownConstants.TICKS_PER_SECOND)) % moveActions.size],
                ),
            ),
        )
    }
...
fun main(args: Array<String>) {
    ClientFramework("localhost", EnigmaShowdownConstants.PORT_SERVER, ZigZagMoveAI).start()
}
