        ...
        if (levelStateBroadcast.ticksUntilBegin != 0) {
            ...
            client.send(
                PlayerActionRequest(
                    ...
                    } else if (state.tick < 39.0) {
                        ConquestAction(
                            moveAction = MoveAction(Math.toRadians(90.0), 10.0),
                        )
                    } else if (state.tick < 50.0) {
                        ConquestAction(
                            moveAction = MoveAction(Math.toRadians(180.0), 10.0),
                        )
                    } else if (state.tick < 61.0) {
                        ConquestAction(
                            moveAction = MoveAction(Math.toRadians(270.0), 10.0),
                        )
                    } else if (state.tick < 66.0) {
                        ConquestAction(
                            moveAction = MoveAction(Math.toRadians(0.0), 10.0),
                        )
                    } else {
...
fun main(args: Array<String>) {
    ClientFramework("localhost", EnigmaShowdownConstants.PORT_SERVER, Level1AI).start()
}
